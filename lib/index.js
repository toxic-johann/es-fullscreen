...
/**
 * es-fullscreen v0.4.0-beta.4
 * (c) 2017-2019 toxic-johann
 * Released under MIT
 * Built ad Sun Jun 02 2019 00:30:28 GMT+0800 (China Standard Time)
 */
...
function native(target, name) {
  ...
  if (name && !lodashEs.isString(name)) {
    option = name;
  }

  if (lodashEs.isString(target)) {
    name = target;
  }
...
function dispatchEvent(element, name) {
  ...
  if (!lodashEs.isPlainObject(event) && !toxicPredicateFunctions.isEvent(event)) {
  ...
  _createClass(ESFullscreen, [{
    ...
    value: function open(element) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (!toxicPredicateFunctions.isPosterityNode(document, element)) {
      ...
      if (!this.useStyleFirst) {
        if (this.isNativelySupport) {
          if (toxicPredicateFunctions.isFunction(element[this._openKey])) {
        ...
        if (element instanceof HTMLVideoElement && element.webkitSupportsFullscreen && toxicPredicateFunctions.isFunction(element.webkitEnterFullscreen)) {
    ...
    value: function _handleEvent(element, behavior, name, fn) {
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (!toxicPredicateFunctions.isElement(element) && element !== document) {
...
exports.default = esFullscreen;
exports.esFullscreen = esFullscreen;
